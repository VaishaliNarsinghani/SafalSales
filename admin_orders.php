...
// Fetch invoices instead of orders
   ...
   <div class="main">
      ...
      <table>
        ...
        <?php while($inv = $invoices->fetch_assoc()){
          // Raw referred_person from DB (no DB changes)
          $refRaw = isset($inv['referred_person']) ? trim((string)$inv['referred_person']) : '';

          // Try to extract a phone number (supports +91, spaces, dashes)
          $refPhone = '';
          if (preg_match('/(\+?\d[\d\s\-]{8,}\d)/', $refRaw, $m)) {
            $refPhone = preg_replace('/\s+/', ' ', trim($m[1]));
          }

          // Name is the text with first phone instance removed
          $refName = $refRaw;
          if ($refPhone !== '') {
            $refName = trim(preg_replace('/'.preg_quote($refPhone, '/').'/', '', $refName, 1));
            $refName = trim(preg_replace('/[-:|,]+$/', '', $refName));
          }
          if ($refName === '' && $refRaw !== '') $refName = $refRaw;
        ?>
        <tr>
          ...
          <td>
            <?php if ($refRaw === '' || $refRaw === null): ?>
              <span class="muted">—</span>
            <?php else: ?>
              <div class="ref-wrap">
                <?php if ($refPhone !== ''): ?>
                  <div class="ref-phone">📞 <?= htmlspecialchars($refPhone) ?></div>
                <?php endif; ?>
                <?php if ($refName === '' && $refPhone === ''): ?>
                  <div><?= nl2br(htmlspecialchars($refRaw)) ?></div>
                <?php endif; ?>
              </div>
            <?php endif; ?>
          </td>
          ...
          <td>
            <a href="invoice_preview.php?id=<?= (int)$inv['id'] ?>" target="_blank" class="btn-action">Download Bill</a>
            <a href="invoice_items.php?id=<?= (int)$inv['id'] ?>" class="btn-action view-btn">👁 View Items</a>
          </td>
        </tr>
        <?php } ?>
      </table>
